...
function BalaoDireito(props: BalaoMensagemProps){
  return (
    <div className={`flex flex-col ${props.omitirAutor && "pr-16"}`}>
      {!!props.omitirAutor && (
        <span className="text-xs text-zinc-600">{props.mensagem.autor}</span>
      )}
      <div className="bg-red-700 text-white px-7 py-4 sm:w-80 rounded-l-3xl rounded-br-3xl">{props.mensagem.texto}</div>
    </div>
  )
}